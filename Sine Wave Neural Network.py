...
def calculate_negative_gradient(L1, L2, L3, A3, correct_answer):
    ...
    for i in range(0, np.shape(L3.weights)[0]):
        for j in range(0, np.shape(L3.weights)[1]):
            prev_neuron = # Figure out
            prev_bias = # FIND OUT HOW TO REFERENCE THIS IN THE RIGHT WAY!!!!!

            dz_dw
            da_dz 
            dco_da
            
            dco_dw = dz_dw * da_dz * dco_da
            negative_gradient.append(dco_dw)
